...
export const SidePanel = () => {
  ...
  return (
    <div className="w-full hidden sm:w-[200px] flex-shrink-0 md:flex sm:flex-col gap-[10px] justify-center sm:justify-start">
      {/* Game Level */}
      {status !== GameStatus.OVER && <div className="bg-[#1D1D1D] rounded-[13.183px] p-2 sm:p-5 border-[0.412px] border-[#3A3A3A]">
        <p className="uppercase text-center text-white font-medium text-xs">level: {gameLevel.current === 'hard' ? 'Difficult' : gameLevel.current === 'medium' ? 'Medium' : 'Easy'}</p>
        <div className="relative mt-2 sm:mt-5">
          <svg className="w-[70px] h-[35px] mx-auto sm:w-[109.133px] sm:h-[54.582px]" viewBox="0 0 266 134" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M133 26.4429C106.015 26.4429 81.3646 36.438 62.5488 52.928C60.9222 54.3536 58.4592 54.3231 56.9297 52.7937L44.497 40.361C42.9084 38.7724 42.9355 36.1826 44.6086 34.6832C68.0564 13.6696 99.0358 0.889526 133 0.889526C166.372 0.889526 196.862 13.2277 220.158 33.5912C221.863 35.0822 221.907 37.6983 220.305 39.3002L207.858 51.7466C206.345 53.2597 203.914 53.3077 202.285 51.9205C183.622 36.0317 159.431 26.4429 133 26.4429Z" className={`${gameLevel.current === 'medium' || gameLevel.current === 'hard' ? 'fill-orange-500' : 'fill-[#292929]'} transition-all duration-200 ease-linear`}></path>
            <path d="M213.587 57.3107C212.058 58.8401 212.027 61.3031 213.453 62.9298C229.289 80.9988 239.134 104.449 239.891 130.179C239.943 131.943 241.368 133.381 243.132 133.381H262.297C264.061 133.381 265.495 131.941 265.453 130.177C264.677 97.4705 252.049 67.698 231.698 44.9895C230.198 43.3164 227.609 43.2893 226.02 44.878L213.587 57.3107Z" className={`${gameLevel.current === 'hard' ? 'fill-red-500' : 'fill-[#292929]'} transition-all duration-200 ease-linear`}></path>
          ...
          <svg viewBox="0 0 46 63" fill="none" xmlns="http://www.w3.org/2000/svg" className="absolute -bottom-1 left-1/2 w-[14px] h-[20px] sm:w-[18.302px] sm:h-[25.7px]" style={{ transform: `translateX(-50%) rotate(${gameLevel.current === 'medium' ? '0' : gameLevel.current === 'hard' ? '90' : '-90'}deg)` }}>
      ...
      <div className="bg-[#1A1A1A] rounded-[13.183px] border-[0.412px] border-[#3A3A3A] inline-block sm:flex-1 overflow-hidden relative">
        <div className="h-full flex flex-col justify-between">
          ...
          (<NextPiecePreview />):
          (<div className="w-full h-full flex flex-col justify-between relative cursor-pointer group"
